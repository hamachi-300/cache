    ...
    if isinstance(cache, LRU) or isinstance(cache, LFU):
        ...
        for key in cache.getKeys():
            ...
            if value:
                ...
                # if not data return ''
                data = item.get('data', '')
                #if not expire_time return 0
                expire_time = item.get('expire_time', 0)
...
def dashboard():
    ...
    # return html page
    return render_template('dashboard.html', stats=cache_stats, cached_items=cached_items, keys=string_keys)
